...
type CamdexEntry = Record<string, "captured" | number>;
type Camdex = Record<string, CamdexEntry>;

const consolidatedCamdex: CamdexEntry = Object.entries(camdex as Camdex).reduce(
  (consolidatedCamdex, [, pokemons]) => {
    const consolidatedCamdexWithPokemonsOfCurrentDate: CamdexEntry = {
      ...consolidatedCamdex,
    };
    Object.entries(pokemons).forEach(([key, value]) => {
      consolidatedCamdexWithPokemonsOfCurrentDate[key] =
        value === "captured" || consolidatedCamdex[key] === "captured"
          ? "captured"
          : Math.min(3, (consolidatedCamdex[key] || 0) + (value || 0));
    });

    return consolidatedCamdexWithPokemonsOfCurrentDate;
  },
  {} as CamdexEntry
);

Object.entries(consolidatedCamdex).forEach(async ([key, value], index) => {
